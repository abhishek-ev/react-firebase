...
function Offers() {
  ...
  const saveData = async () => {
    ...
    await addDoc(collection(db, "offers"), offer).then(() => {
      loadOffers()
  ...
  const deleteItem = async (id) => {
    console.log("id", id)
    setLoader(true)
    await deleteDoc(doc(db, "offers", id)).then(() => {
      loadOffers().then(() => {
        setLoader(false);
      });
    });
    ...
    <div className='position-relative'>
      ...
      <div className="offcanvas offcanvas-end" style={{ "--bs-offcanvas-width": "550px" }} data-bs-backdrop="static" tabIndex="-1" id="staticBackdrop" aria-labelledby="staticBackdropLabel">
        <div className="offcanvas-header">
          <h5 className="offcanvas-title" id="staticBackdropLabel">Add offer</h5>
